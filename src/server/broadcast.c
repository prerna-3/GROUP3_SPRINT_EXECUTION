...
void broadcast_message(const char *message, int sender_id) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (client_sockets[i].socket != 0 && client_sockets[i].socket != sender_id) {

       if (send(client_sockets[i].socket, message, strlen(message), 0) < 0) {
                perror("Send failed");
                log_message(WARN, "broadcast.c", "Failed to send message to client socket %d", client_sockets[i].socket);
            }
        }
    }
