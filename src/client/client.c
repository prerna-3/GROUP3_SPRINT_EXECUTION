...
void login_or_register(int sockfd) {
    ...
    if (choice != 1 && choice != 2) {
        ...
        log_message(WARN, "client.c", "invalid choice");
        exit(1);
    }
    ...
    if (choice == 1 && strcmp(buffer, "Login successful") == 0) {
        ...
        if (strcmp(buffer, "Registration successful") == 0) {
            ...
            close(sockfd);
            exit(0);
        } else if (strcmp(buffer, "User already registered") == 0) {
            printf("Registration failed: User already registered.\n");
            log_message(WARN, "client.c", "Registration failed: User already registered");
            exit(1);
        }
    } else {
